import { KeyRound, Sparkles, BookOpen, HelpCircle, Moon, Sun, ExternalLink, Settings, ArrowLeftRight, BookMarked } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center justify-between w-full sm:w-auto">
          ...
          <div className="flex items-center gap-1.5 sm:hidden">
            {/* Mobile Notebook Drawer Trigger */}
            <button
              type="button"
              id="btn-notebook-mobile"
              onClick={toggleDrawer}
              className="relative p-1.5 rounded-lg text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-stone-800 transition-colors cursor-pointer"
              title="فتح دفتر الكلمات والشيفرات"
            >
              <BookMarked className="w-5 h-5" />
              {entries.length > 0 && (
                <span className="absolute -top-1 -right-1 min-w-4 h-4 px-1 rounded-full bg-amber-600 text-white font-mono text-3xs font-black flex items-center justify-center">
                  {entries.length}
                </span>
              )}
            </button>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto flex-wrap">
          ...
          {/* Notebook Trigger Button (Desktop) */}
          <button
            type="button"
            id="btn-notebook-desktop"
            onClick={toggleDrawer}
            className="hidden sm:inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-800 dark:text-amber-300 border border-amber-300/80 dark:border-amber-700/60 font-bold text-xs transition-all cursor-pointer shadow-2xs hover:scale-102"
            title="فتح دفتر الكلمات والشيفرات المحفوظة"
          >
            <BookMarked className="w-4 h-4 text-amber-600 dark:text-amber-400" />
            <span>الدفتر</span>
            {entries.length > 0 && (
              <span className="min-w-4.5 h-4.5 px-1.5 rounded-full bg-amber-600 text-white font-mono text-3xs font-black flex items-center justify-center">
                {entries.length}
              </span>
            )}
          </button>
