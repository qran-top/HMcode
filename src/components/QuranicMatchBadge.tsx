  ...
  if (exactMeta) {
    ...
    if (compact) {
      return (
        <a
          ...
          className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-2xs font-extrabold bg-amber-500/15 hover:bg-amber-500/25 text-amber-900 border border-amber-400 transition-colors group cursor-pointer"
          title={
            isMultiple
        ...
        >
          ...
          <ExternalLink className="w-2.5 h-2.5 text-amber-800 opacity-60 group-hover:opacity-100 transition-opacity shrink-0" />
        </a>
      );
    }
    ...
    return (
      <a
        ...
        rel="noopener noreferrer"
        onClick={(e) => e.stopPropagation()}
        className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-bold bg-amber-50 hover:bg-amber-100 text-amber-950 border border-amber-300 shadow-2xs transition-all group cursor-pointer"
        title={
          isMultiple
      ...
      >
        ...
        <ExternalLink className="w-3 h-3 text-amber-700 opacity-70 group-hover:opacity-100 transition-opacity shrink-0" />
        <span className="text-2xs bg-amber-200/80 px-1 py-0.5 rounded text-amber-900 font-semibold mr-1">
  ...
  if (showNearest && nearestMeta && nearestMeta.similarity >= 60) {
    ...
    if (compact) {
      return (
        <a
          ...
          rel="noopener noreferrer"
          onClick={(e) => e.stopPropagation()}
          className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-2xs font-semibold bg-stone-100 hover:bg-stone-200 text-stone-700 border border-stone-200 transition-colors group cursor-pointer"
        ...
        >
          ...
          <ExternalLink className="w-2.5 h-2.5 text-stone-500 opacity-60 group-hover:opacity-100 shrink-0" />
        </a>
      );
    }
    ...
    return (
      <a
        ...
        rel="noopener noreferrer"
        onClick={(e) => e.stopPropagation()}
        className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-2xs font-medium bg-stone-50 hover:bg-stone-100 text-stone-700 border border-stone-200 transition-all group cursor-pointer"
        title={`بحث عن أقرب لفظ قرآني "${nearestMeta.originalQuranicWord}" على موقع قرآن توب`}
      >
        <Sparkles className="w-3 h-3 text-amber-600 shrink-0" />
        <span>
          أقرب لفظ قرآني: <strong className="text-stone-900 font-bold">{nearestMeta.originalQuranicWord}</strong>
        </span>
        <span className="text-stone-500 font-mono text-3xs mr-1">
          ({nearestMeta.similarity}% - سورة {nearestMeta.surahName} آية {nearestMeta.ayahNum})
        </span>
        <ExternalLink className="w-2.5 h-2.5 text-stone-400 group-hover:text-stone-700 shrink-0" />
      </a>
    );
  }
