...
export function DecryptView({
  ...
  const { layers: cipherLayers, validCipherLetters, activeTableName } = useCipherLayers();
      ...
      {!isDualMode && ( <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-xs p-3.5 sm:p-5 max-w-full overflow-hidden transition-colors">
        ...
        <div className="flex flex-col-reverse sm:flex-row items-stretch gap-2.5 w-full max-w-full">
          ...
          <div className="shrink-0 flex flex-row sm:flex-col gap-1.5 w-full sm:w-36">
            ...
            {/* Search & Show Combinations button under Clear */}
            <button
              type="button"
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-1">
                      ...
                      <div className="flex items-center gap-0.5 shrink-0">
                        ...
                        <AddToNotebookButton
                          word={quranicMeta?.originalQuranicWord || item.word}
                          cipher={cipherInput}
                          systemName={activeTableName || undefined}
                          surahInfo={quranicMeta?.surahName}
                          ayahNum={quranicMeta?.ayahNum}
                          isReversed={item.isReversed}
                          type={quranicMeta ? 'quranic' : isDictWord ? 'dictionary' : 'decryption'}
                          variant="icon-only"
                        />
                      </div>
                    </div>
