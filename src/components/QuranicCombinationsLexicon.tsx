...
export function QuranicCombinationsLexicon({
  ...
  return (
    <div
      ...
      className="bg-linear-to-b from-amber-50/70 to-emerald-50/50 rounded-2xl border-2 border-amber-300/80 shadow-xs overflow-hidden transition-all"
    >
      {/* Header Bar */}
      ...
      {isExpanded && (
        <div className="p-3 sm:p-4 space-y-4">
          ...
          {!hasGenerated && !isGenerating && onGenerate && (
            <div className="p-4 sm:p-5 rounded-xl border border-dashed border-amber-300 bg-white/90 text-center space-y-3">
          ...
          {isGenerating && (
            <div className="p-4 rounded-xl bg-white/80 border border-amber-200 text-center space-y-2">
              <div className="flex items-center justify-center gap-2 text-xs sm:text-sm font-bold text-amber-900">
                ...
                <div className="pt-3 border-t border-amber-200/60 space-y-2">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-1.5 text-xs font-bold text-stone-800">
                      ...
                      <span className="bg-amber-100 text-amber-900 text-2xs font-extrabold px-2 py-0.5 rounded-full border border-amber-300">
                    ...
                    {nearestMatches.length > 10 && (
                      <button
                        ...
                        className="text-2xs font-bold text-amber-800 hover:text-amber-950 underline cursor-pointer"
                      >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
                    {displayedNearest.map(({ combo, nearest }, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className="bg-white rounded-xl border border-amber-200 hover:border-amber-400 p-2.5 text-xs flex flex-col justify-between gap-2 shadow-2xs hover:shadow-xs transition-all"
                        >
                          <div className="flex items-center justify-between gap-1">
                            <div className="flex items-center gap-1">
                              <span className="font-extrabold text-stone-700 tracking-wider font-mono">
                                {combo}
                              </span>
                          ...
                          <div className="flex items-center justify-between gap-1 text-2xs pt-1.5 border-t border-stone-100">
                            <a
                              href={getQuranTopSearchUrl(nearest.originalQuranicWord || nearest.word)}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-stone-500 hover:text-amber-800 hover:underline inline-flex items-center gap-1 group/link"
