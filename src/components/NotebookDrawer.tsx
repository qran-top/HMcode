...
import { NotebookWordsQuiz } from './NotebookWordsQuiz';
import {
  BookMarked,
  X,
  Trash2,
...
export function NotebookDrawer() {
  ...
  // Helper to populate sample words for testing quiz and sentence builder
  const handleAddSampleQuizData = () => {
    // Original Word 1: الله -> results
    addEntry('نور', 'الله', { type: 'quranic', surahInfo: 'سورة النور' });
    addEntry('حق', 'الله', { type: 'quranic', surahInfo: 'سورة الحج' });
    addEntry('سلام', 'الله', { type: 'dictionary' });

    // Original Word 2: خلق -> results
    addEntry('أبدع', 'خلق', { type: 'dictionary' });
    addEntry('برأ', 'خلق', { type: 'quranic', surahInfo: 'سورة الحشر' });
    addEntry('صور', 'خلق', { type: 'dictionary' });

    // Original Word 3: السماوات -> results
    addEntry('العرش', 'السماوات', { type: 'quranic', surahInfo: 'سورة المؤمنون' });
    addEntry('الملكوت', 'السماوات', { type: 'quranic', surahInfo: 'سورة الأنعام' });
    addEntry('الآفاق', 'السماوات', { type: 'quranic', surahInfo: 'سورة فصلت' });
        ...
        <div className="fixed inset-0 z-50 overflow-hidden" dir="rtl">
          ...
          <div className="absolute inset-y-0 right-0 max-w-full flex pl-0 sm:pl-10">
            <div className="w-screen max-w-xl bg-white dark:bg-stone-900 shadow-2xl border-l border-stone-200 dark:border-stone-800 flex flex-col h-full animate-in slide-in-from-right duration-250">
              ...
              {/* Main Tabs: Systems vs Current Words vs Quiz & AI vs Saved Pages */}
              <div className="flex items-center border-b border-stone-200 dark:border-stone-800 bg-stone-100/70 dark:bg-stone-950/50 p-1 gap-1">
                ...
                <button
                  ...
                  title="كويز وتوليف الجمل وبرومبت الذكاء الاصطناعي"
                >
                  <Bot className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400 shrink-0" />
                  <span className="truncate">كويز وAI</span>
                </button>
              ...
              {drawerTab === 'entries' && (
                <>
                  ...
                  {entries.length > 0 && (
                    <div className="p-2 bg-stone-50/95 dark:bg-stone-950/90 border-t border-stone-200 dark:border-stone-800 flex items-center justify-between gap-1.5 flex-wrap">
                      <div className="flex items-center gap-1 text-2xs font-bold flex-wrap">
                        <button
                          type="button"
                          onClick={() => setDrawerTab('quiz')}
                          className="py-1 px-2 rounded-md bg-indigo-600 hover:bg-indigo-700 text-white inline-flex items-center gap-1 transition-colors cursor-pointer shadow-2xs font-bold"
                          title="فتح كويز وتوليف الجمل وبرومبت الذكاء الاصطناعي"
                        >
                        ...
                        <button
                          type="button"
                          onClick={() => handleCopy(generateAIPrompt(entries), 'copy_ai_prompt')}
                          className="py-1 px-2 rounded-md bg-amber-50 dark:bg-amber-950/60 border border-amber-300 dark:border-amber-800 text-amber-900 dark:text-amber-200 hover:bg-amber-100 dark:hover:bg-amber-900/80 inline-flex items-center gap-1 transition-colors cursor-pointer shadow-2xs"
                          title="نسخ برومبت الذكاء الاصطناعي لتأليف جملة من النتائج"
                        >
                          {copiedKey === 'copy_ai_prompt' ? (
                            <Check className="w-3 h-3 text-emerald-600" />
                          ) : (
                        ...
                        <button
                          ...
                          className="py-1 px-1.5 rounded-md bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-700 text-stone-700 dark:text-stone-200 hover:bg-stone-100 dark:hover:bg-stone-700 inline-flex items-center gap-1 transition-colors cursor-pointer shadow-2xs"
                          title="تصدير ملف TXT"
                        >
              ...
              {/* TAB: QUIZ & AI PROMPT BUILDER */}
              {drawerTab === 'quiz' && (
                <div className="flex-1 flex flex-col overflow-hidden">
                  <NotebookWordsQuiz
                    entries={entries}
                    onAddSampleData={handleAddSampleQuizData}
                  />
                </div>
              )}
                          ...
                          >
                            ...
                            {isSelected && (
                              <div className="p-2 border-t border-indigo-200 dark:border-indigo-900/60 bg-white/70 dark:bg-stone-900/70 space-y-2">
                                ...
                                <div className="flex items-center justify-end gap-1 pt-1 border-t border-stone-200/50 dark:border-stone-800/50 text-3xs flex-wrap">
                                  <button
                                    type="button"
                                    onClick={() => handleCopy(generateAIPrompt(page.entries), `prompt_page_${page.id}`)}
                                    className="px-2 py-0.5 rounded bg-amber-50 dark:bg-amber-950/60 text-amber-900 dark:text-amber-200 hover:bg-amber-100 dark:hover:bg-amber-900 border border-amber-300 dark:border-amber-800 inline-flex items-center gap-1 cursor-pointer font-bold"
                                    title="نسخ برومبت الذكاء الاصطناعي لتأليف جملة من كلمات هذه الصفحة"
                                  >
                                    {copiedKey === `prompt_page_${page.id}` ? (
