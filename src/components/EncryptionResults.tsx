...
import { Copy, Check, Shuffle, RefreshCw, Layers, Loader2, AlertCircle, ArrowLeftRight, Sparkles, BookOpen, SpellCheck, Repeat, ExternalLink } from 'lucide-react';
...
export function EncryptionResults({
  ...
  const [onlyPureNoorani, setOnlyPureNoorani] = useState<boolean>(false);
  const [onlyClosedLoop, setOnlyClosedLoop] = useState<boolean>(false);
  const [onlyMirrorTwin, setOnlyMirrorTwin] = useState<boolean>(false);
  ...
  // The actual searched Arabic word (entered by user to be encrypted)
  const searchedWord = details.map((d) => d.originalChar).join('').trim();
  const searchedWordClean = cleanText(searchedWord);
  ...
  // Noorani Purity Map (14 letters index)
  const purityMap = useMemo(() => {
    const map = new Map<string, ReturnType<typeof getNooraniPurity>>();
    for (const item of processedCombinations) {
      if (!map.has(item.word)) {
        map.set(item.word, getNooraniPurity(item.word));
      }
    }
    return map;
  }, [processedCombinations]);

  // Mirror & Palindrome Symmetry Map
  const mirrorMap = useMemo(() => {
    const map = new Map<string, ReturnType<typeof analyzeMirrorSymmetry>>();
    for (const item of processedCombinations) {
      if (!map.has(item.word)) {
        map.set(item.word, analyzeMirrorSymmetry(item.word));
      }
    }
    return map;
  }, [processedCombinations]);

  // Closed Reciprocal Loop Map
  const loopMap = useMemo(() => {
    const map = new Map<string, ReturnType<typeof checkBidirectionalLoop>>();
    for (const item of processedCombinations) {
      if (!map.has(item.word)) {
        map.set(
          item.word,
          checkBidirectionalLoop(searchedWord || item.original, item.word, layers, 'encryption')
        );
      }
    }
    return map;
  }, [processedCombinations, searchedWord, layers]);
  ...
  // Filtering by substring, Quranic vocabulary, Arabic dictionary, multi-letter opening words, purity, loop, and mirror
  ...
  const filteredCombinations = useMemo(() => {
    return sortedCombinations.filter((item) => {
      ...
      if (onlyPureNoorani) {
        const purity = purityMap.get(item.word);
        if (!purity || !purity.isPure) return false;
      }
      if (onlyClosedLoop) {
        const loop = loopMap.get(item.word);
        if (!loop || !loop.isClosedLoop) return false;
      }
      if (onlyMirrorTwin) {
        const mirror = mirrorMap.get(item.word);
        if (!mirror || (!mirror.isTwinMatch && !mirror.isPalindrome)) return false;
      }
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 1. Multi-Word Coherence Scanner for Phrases */}
      {!isDualMode && searchedWord.split(/\s+/).filter(Boolean).length >= 2 && (
        <MultiWordCoherenceCard
          inputText={searchedWord}
          layers={layers}
          mode="encryption"
        />
      )}
      ...
      <div id="all-combinations-card" className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-xs p-4 sm:p-5 space-y-3">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 pb-3 border-b border-stone-100 dark:border-stone-800">
          ...
          <div className="flex items-center gap-2 flex-wrap w-full lg:w-auto">
            ...
            {/* Filter: 100% Pure Noorani 14 Letters */}
            <button
              type="button"
              id="pure-noorani-btn"
              onClick={() => setOnlyPureNoorani(!onlyPureNoorani)}
              className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg border transition-all cursor-pointer ${
                onlyPureNoorani
                  ? 'bg-amber-600 text-white border-amber-700 shadow-2xs ring-2 ring-amber-300 dark:ring-amber-700'
                  : 'bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300 border-stone-200 dark:border-stone-700 hover:bg-amber-50 dark:hover:bg-amber-950/40 hover:border-amber-300'
              }`}
              title="عرض الاحتمالات المبنية حصراً بنسبة 100% من الحروف النورانية الـ 14 المشتركة"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-500" />
              <span>نورانية 100% (14 حرفاً)</span>
            </button>

            {/* Filter: Bidirectional Closed Loop */}
            <button
              type="button"
              id="closed-loop-btn"
              onClick={() => setOnlyClosedLoop(!onlyClosedLoop)}
              className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg border transition-all cursor-pointer ${
                onlyClosedLoop
                  ? 'bg-purple-600 text-white border-purple-700 shadow-2xs ring-2 ring-purple-300 dark:ring-purple-700'
                  : 'bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300 border-stone-200 dark:border-stone-700 hover:bg-purple-50 dark:hover:bg-purple-950/40 hover:border-purple-300'
              }`}
              title="عرض الاحتمالات التي تشكل دورة تبادلية مغلقة تعيد نفس الأصل بدقة"
            >
              <Repeat className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" />
              <span>حلقة مغلقة ♾️</span>
            </button>

            {/* Filter: Mirror & Twin Palindrome */}
            <button
              type="button"
              id="mirror-twin-btn"
              onClick={() => setOnlyMirrorTwin(!onlyMirrorTwin)}
              className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg border transition-all cursor-pointer ${
                onlyMirrorTwin
                  ? 'bg-cyan-700 text-white border-cyan-800 shadow-2xs ring-2 ring-cyan-300 dark:ring-cyan-700'
                  : 'bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300 border-stone-200 dark:border-stone-700 hover:bg-cyan-50 dark:hover:bg-cyan-950/40 hover:border-cyan-300'
              }`}
              title="عرض الاحتمالات ذات التناظر الذاتي (Palindrome) أو التي لمعكوسها معنى أيضاً"
            >
              <ArrowLeftRight className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400" />
              <span>تناظر مرآتي ⇄</span>
            </button>
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-1">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <AddToNotebookButton
                        word={searchedWord || (exactMeta ? exactMeta.word : typeof isDictWord === 'string' ? isDictWord : item.word)}
                        cipher={exactMeta ? exactMeta.word : typeof isDictWord === 'string' ? isDictWord : item.word}
                        systemName={activeTableName || undefined}
                        surahInfo={exactMeta?.surahName}
                        ayahNum={exactMeta?.ayahNum}
                        isReversed={item.isReversed}
                        type={exactMeta ? 'quranic' : isDictWord ? 'dictionary' : 'encryption'}
                        variant="icon-only"
                      />
                    </div>
                  </div>

                  {/* Badges & Quranic / Dictionary Vocabulary Match Details */}
                  <div className="space-y-1.5">
                    {/* Advanced Analysis Badges: Purity, Loop, Mirror */}
                    <div className="flex items-center gap-1 flex-wrap">
                      {purity && <NooraniPurityBadge purity={purity} />}
                      {loop && loop.isClosedLoop && <BidirectionalLoopBadge isClosedLoop={true} />}
                      {mirror && (mirror.isTwinMatch || mirror.isPalindrome) && (
                        <MirrorSymmetryBadge mirror={mirror} />
                      )}
                    </div>
                    ...
                    {isDictWord && !exactMeta && (
                      ...
                      >
                        ...
                        <ExternalLink className="w-2.5 h-2.5 text-emerald-600 dark:text-emerald-400 opacity-60 group-hover:opacity-100 transition-opacity shrink-0" />
                      </a>
                    )}
