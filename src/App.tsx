...
export function App() {
  ...
  // Ensure title is always 'التشفير العربي' across all views and browser tabs
  useEffect(() => {
  ...
  const encryptNooraniAnalysis = useMemo(() => {
    const raw = inputText.trim().replace(/[^ء-ي]/g, '');
    if (!raw) return null;
    const chars = raw.split('');
    const nooraniCount = chars.filter((c) => NOORANI_LETTERS_SET.has(c)).length;
    const isPure = nooraniCount === chars.length;
    return {
      isPure,
      nooraniCount,
      total: chars.length,
      nonNooraniCount: chars.length - nooraniCount,
    };
  }, [inputText]);
          ...
          <div className="bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-xs p-3.5 sm:p-5 max-w-full overflow-hidden transition-colors">
            ...
            <div className="flex flex-col-reverse sm:flex-row items-stretch gap-2.5 w-full max-w-full">
              ...
              <div className="shrink-0 flex flex-row sm:flex-col gap-1.5 w-full sm:w-36">
                ...
                {/* Search & Show Combinations button */}
                <button
                  type="button"
                  ...
                  title="بحث وعرض قائمة الاحتمالات (أو اضغط Enter في مربع النص)"
                >
                  {encryptGenStatus.isGenerating ? (
                    <>
                    ...
                    <>
                      <Search className="w-3.5 h-3.5 text-amber-200 shrink-0" />
                      <span>بحث وعرض الاحتمالات</span>
                    </>
                  )}
                </button>
              </div>
            ...
            {/* Smart Noorani Guidance Banner */}
            {encryptNooraniAnalysis && (
              <div
                className={`mt-2.5 flex items-center justify-between text-2xs px-3 py-1.5 rounded-lg border transition-all ${
              ...
              >
                <div className="flex items-center gap-2 font-bold flex-wrap">
                  ...
                  >
                    {encryptNooraniAnalysis.isPure ? <Unlock className="w-2.5 h-2.5" /> : <Lock className="w-2.5 h-2.5" />}
                  </div>
                  <span>
                    {encryptNooraniAnalysis.isPure
                      ? `أحرف نورانية خالصة (${encryptNooraniAnalysis.nooraniCount} من 14)`
                      : `أحرف عامة (${encryptNooraniAnalysis.nonNooraniCount} غير نوراني من 28)`}
                  </span>
