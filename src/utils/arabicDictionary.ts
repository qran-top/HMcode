...
class DictionaryService {
  ...
  getMatchedWord(rawWord: string): string | null {
    ...
    if (/(.)\1/.test(clean)) {
      ...
      if (this.wordSet.has(collapsed)) return collapsed;
      const normCol = collapsed
        .replace(/[أإآ]/g, 'ا')
        .replace(/ى/g, 'ي')
        .replace(/ة/g, 'ه');
      if (this.wordSet.has(normCol)) return normCol;
      const cbCol = getCipherBaseString(collapsed);
      if (this.cipherBaseMap.has(cbCol)) return this.cipherBaseMap.get(cbCol) || null;
    }
